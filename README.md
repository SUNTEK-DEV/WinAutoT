...
1. 用 USB 转串口把设备 LED 控制器接到电脑（Windows 上一般为 `COMx`）。
2. 启动工具，点 **刷新/探测**，选中串口后点 **连接**。
   - 工具会发 `VERSION`，应答以 `FY` 开头即识别为 LED 控制器。
   - 部分固件不支持版本查询，可选择强行连接。
3. 填写整机编号、操作员（可选）。
4. 确认录音 / 播放设备（默认系统设备即可）。
5. 点 **全部测试**：
   - LED / 补光灯 / 扬声器 / 相机：灯、声音或预览画面保持到弹窗确认 → 是=PASS，否=FAIL。
   - 麦克风回环：自动判定（须检测到 1 kHz 测试音）。
6. 导出 TXT 或 CSV 报告到 `reports/`。

未接 LED 串口时，点「全部测试」可选择只跑音频项，LED 项记为「跳过」。含跳过项的报告总判定为「跳过」，不会给出 PASS。
